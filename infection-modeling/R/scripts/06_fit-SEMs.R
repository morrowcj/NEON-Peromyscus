...
alt_both_sem_max <- psem(
  tol_alt2 %>%
    update(. ~ . - Bb_infected + sex_male + ticks_attached + avg_move_dist,
           # + log_burden,
           data = full_dat),
  burden_alt %>%
    update(
      . ~ . + Bb_infected - ticks_attached + wthr_PC1 + expr_PC2,
      data = full_dat #%>% filter(Bb_infected > 0)
    ),
  infect_alt %>% update(. ~ . - weight + expr_PC2, data = full_dat),
  res_alt %>% update(data = full_dat),
  tick_alt %>%
    update(
    . ~ . + sex_mature + weight + cap_prop_night + weighted_trapability
...
tol_bhv <- tol_alt2 %>%
  update(. ~ . - weighted_trap_diversity + behave_PC1 + behave_PC2 + wthr_PC2)
burden_bhv <- burden_alt %>%
  update(. ~ . + behave_PC1 + behave_PC2 + Bb_infected + sex_mature)
infect_bhv <- infect_alt %>% update(. ~ . + behave_PC1 + behave_PC2)
res_bhv <- res_alt %>% update(. ~ . + behave_PC1 + behave_PC2 + wthr_PC2)
tick_bhv <- tick_alt %>%
  update(. ~ . - weighted_trap_diversity + behave_PC1 + behave_PC2 + wthr_PC2)
behave1_full <- lmer(
  behave_PC1 ~ sex_male*sex_mature*weight +
    wthr_PC1 + wthr_PC2 + clim_PC1 + clim_PC2 +
    (1|siteID) + (1|year),
  data = mod_dat
)
behave1_bhv <- behave1_full %>%
  update(
    . ~ . - clim_PC2 - sex_male:sex_mature:weight - sex_male:sex_mature -
      clim_PC1
  )
behave2_full <- update(behave1_full, behave_PC2 ~ .)
behave2_bhv <- behave2_full %>%
  update(
    . ~ . - sex_male:sex_mature:weight - sex_mature:weight - clim_PC2 -
      clim_PC1
  )

bhv_both_sem_max <- psem(
  tol_bhv,
  burden_bhv,
  infect_bhv,
  res_bhv,
...
bhv_both_sem_max_smry <- summary(bhv_both_sem_max)
bhv_both_sem_max_smry$dTable %>% data.frame() %>%
  filter(P.Value <= 0.1) %>%
  select(-c(Test.Type, Crit.Value, DF))
bhv_both_sem_max_smry$coefficients %>% data.frame() %>%
  select(Response, Predictor, P.Value, Var.9)
saveRDS(
...
## ---- Swap tolerance again ----

bhvalt_both_sem_max <- psem(
  tol_bhv %>% update(. ~ . - Bb_infected + sex_male),
  burden_bhv %>% update(. ~ . + expr_PC2),
  infect_bhv %>% update(. ~ . + expr_PC2),
  res_bhv,
  tick_bhv %>% update(. ~ . + expr_PC2),
  behave1_bhv,
  behave2_bhv
)
bhvalt_both_sem_max_smry <- summary(bhvalt_both_sem_max)
bhvalt_both_sem_max_smry$dTable %>% data.frame() %>%
  filter(P.Value <= 0.1) %>%
  select(-c(Test.Type, Crit.Value, DF))
bhvalt_both_sem_max_smry$coefficients %>% data.frame() %>%
  select(Response, Predictor, P.Value, Var.9)
saveRDS(
  bhvalt_both_sem_max_smry,
  file.path(mod_output_dir, "alt-behaviorPCA-BurdenInfection-SEM_maximal-cases.rds")
)
