...
```{r}
# set global neon token variable
source("data-curation/R/00_NEON-API-token.R")
.setNeonTokenGlobal("NEON-API-token-morrow5.txt")
```
...
# add in bout eventID
mammals <- mammal_prods$mam_perplotnight |> 
  tibble() |> 
  mutate(
  ...
  right_join(mammals, by = c("nightuid", "siteID", "plotID"))
...
Let's look at the number of bouts per site per year:

```{r}
mammals |> 
  group_by(year, siteID) |> 
  summarize(n_bouts = length(unique(boutID))) |> 
  ggplot(aes(x = year, y = n_bouts, fill = siteID)) + 
  geom_col(position = "dodge", col = "black") +
  scale_y_continuous(breaks = seq(0, 10, by = 2))
```

On average, it seems that there are 4 trapping bouts per year at most sites.

And it also appears that the same number of plots are usually 
included in each bout within a site:

```{r}
mammals |> 
  group_by(year, siteID, boutID) |> 
  summarize(plots_per_bout = length(unique(plotID))) |> 
  ggplot(aes(x = year, y = plots_per_bout, col = siteID)) + 
  stat_summary(size = 0.33) +
  stat_summary(geom = "line", aes(group = siteID)) + 
  scale_y_continuous(breaks = seq(0, 10, by = 2)) + 
  scale_x_continuous(breaks = seq(2000, 2030, by = 2))
```

and while the bout lengths vary, they are generally 2-3 days long, with 
some notable outliers:

```{r}
# add information about bout length
mammals <- mammals |> 
  group_by(siteID, boutID) |> 
  arrange(boutID, date) |> 
  summarize(
    boutStart = first(date), boutEnd = last(date), 
    boutLength = time_length(boutEnd - boutStart, unit = "day"),
    .groups = "drop"
  ) |> 
  right_join(mammals, by = c("siteID", "boutID"))

# plot bout length
mammals |> 
  filter(boutLength <= 100) |>  # remove outlier
  ggplot(aes(y = boutLength, x = siteID)) +
  scale_y_continuous(breaks = seq(0, 10, by = 2)) + 
  geom_boxplot() +
  labs(y = "bout length (days)")
```
...
ggsave(
  "infection-modeling/graphics/cap_path_counts.jpg", 
...
ggsave(
  "infection-modeling/graphics/nocap_path_counts.jpg", 
  width = 4.5, height = 4.5
)
...
ggsave(
  "infection-modeling/graphics/cap_nopath_counts.jpg", 
  width = 4.5, height = 4.5
)
...
ggsave(
  "infection-modeling/graphics/nocap_nopath_counts.jpg", 
  width = 4.5, height = 4.5
)
...
ticks <- tick_obj$tck_taxonomyProcessed |> tibble() |> 
  mutate(
    ...
    date = date(collectDate),
    dateTime = as_datetime(collectDate),
    week = week(collectDate),
    siteID = gsub("(.*)_.*", "\\1", plotID)
  ) |> 
  filter(!is.na(acceptedTaxonID)) # remove something that isn't a tick

n_ticks = nrow(ticks)

# get bout and field observation info, to attach to the tick table
fieldticks <- tick_obj$tck_fielddata |> tibble() |> 
  filter(!is.na(sampleID)) |>  # remove rows without corresponding lab count
  mutate(year = year(collectDate), DateTime = as_datetime(collectDate)) |> 
  arrange(plotID, DateTime) |> 
  mutate(
    eventID = factor(eventID, levels = unique(eventID)),
  ) |> 
  select(
    plotID, eventID, sampleID, collectDate,
    fieldAdultCount = adultCount, fieldLarvaCount = larvaCount, 
    fieldNymphCount = nymphCount
  )

# join with the tick table
ticks <- fieldticks |>   
  right_join(ticks, by = c("plotID", "sampleID", "collectDate")) |> 
  tibble()
  ...
  #   tck_samp_days = length(unique(collectDate))
  # ) |> 
  # ungroup() |> 
  # mutate(
...
```{r}
tick_densities <- ticks |> 
  mutate(genus = replace_na(genus, "UNK")) |> 
  group_by(siteID, eventID) |> 
  mutate(
    eventStart = first(dateTime), eventEnd = last(dateTime),
    sweeps = length(unique(interaction(plotID, dateTime))),
  ) |> 
  group_by(siteID, eventID, eventStart, eventEnd, genus, sexOrAge) |> 
  arrange(date) |> 
  summarize(
    count = sum(individualCount),
    sweeps = unique(sweeps),
    density = count / sweeps, 
    .groups = "drop"
  ) |> 
  relocate(eventStart, eventEnd, .after = eventID)

# create a table with all the unique genus and age combos
genusAges <- tick_densities |> 
  select(eventID, genus, sexOrAge) |> 
  distinct() |> 
  complete(eventID, genus, sexOrAge)

# add in sweeps sweeps
genusAges <- tick_densities |> 
  select(siteID, eventID, eventStart, eventEnd, sweeps) |> 
  distinct() |> 
  full_join(genusAges, by = "eventID")
  
# add these combos back into the tick densities
tick_densities <- tick_densities |> 
  full_join(genusAges) |> 
  mutate(
    count = replace_na(count, replace = 0),
    density = replace_na(density, replace = 0)
  ) |> arrange(siteID, eventStart)

tick_densities
```

Do the mammal trapping events line up with the tick trapping events? The below
figure suggests "yes"!

```{r}
tick_densities |> 
  select(eventID, eventStart, eventEnd) |> 
  distinct() |> 
  inner_join(
    mammals |> select(boutID, boutStart, boutEnd), by = c("eventID" = "boutID")
  ) |> 
  mutate(year = year(eventStart)) %>%
  filter(complete.cases(.)) |> 
  ggplot(aes(x = eventStart, y = boutStart)) + 
  facet_wrap(~year, scales = "free") +
  geom_abline(intercept = 0, slope = 1, col = "blue") + 
  geom_point() + 
  labs(x = "Tick bout start", y = "Mammal bout start") 
```
...
## Factors affecting tick attachment
### Sex and Age

```{r, fig.asp=0.33, fig.width=7}
full_data |> 
  filter(
    taxonID %in% c("PEMA", "PELE", "PELEPEMA"),
    sex != "U", !is.na(sex), lifeStage != "unknown",
  ) |> 
  group_by(larvalTicksAttached, sex) |> 
  pivot_longer(
    c(larvalTicksAttached, nymphalTicksAttached, adultTicksAttached),
    names_to = "tickStage", values_to = "attached"
  ) |> 
  filter(!is.na(attached)) |> 
  mutate(
    sexM = as.numeric(sex == "M"),
    tickStage = gsub("TicksAttached", "", tickStage),
    attached = as.numeric(attached == "Y")
  ) |> 
  ggplot(aes(x = sex, y = attached, col = lifeStage)) + 
  facet_rep_wrap(~tickStage, scales = "free_y") +
  stat_summary(
    fun.data = mean_cl_normal, size = 0.3, position = position_dodge(.2)
  ) + 
  stat_summary(fun = "mean",
    geom = "line", aes(group = lifeStage), 
    alpha = 0.5, linetype = "dashed"
  ) +
  labs(y = "Proporiton of mice\nwith ticks (± SE)", x = "Sex",
       col = "Age") + 
  theme_bw() + 
  theme(strip.background = element_blank())

ggsave(
  "infection-modeling/graphics/tick-attachment-by-sex-and-age.jpg",
  width = 7, height = 7*.33, dpi = 200
)
```

* Among adults, males more likely to have all tick life stages attached 

* Among subadults, males are slightly more likely to have adult and nymphal 
ticks attached, but females are sligthly more likely to have larval ticks
attached.

* Among jeveniles, females are slightly (likely insignificant) more likely to
have ticks attached
...
full_data |> 
  ...
  ggplot(aes(x = weight, y = attached)) + 
  facet_rep_wrap(~tickStage, scales = "free_y") +
  geom_point(size = 1) + 
  geom_smooth(method = "glm", method.args = list(family = "binomial")) + 
  labs(y = "Ticks are attached\n(logistic regression)", x = "Weight",
       col = "Age") + 
  theme_bw() + 
  theme(strip.background = element_blank())

ggsave(
  "infection-modeling/graphics/tick-attachment-by-weight.jpg",
  width = 7, height = 7*.33, dpi = 200
)
```

And larger individuals are more likely to have ticks attached (at least 
larvae and nymphs).
...
full_data |> 
  ...
  ggplot(aes(x = cap_prop_night, y = attached)) + 
  facet_rep_wrap(~tickStage, scales = "free_y") +
  geom_point(size = 1) + 
  geom_smooth(method = "glm", method.args = list(family = "binomial")) + 
  labs(
    y = "Ticks are attached\n(logistic regression)", 
    x = "Relative capture time",
    col = "Age"
  ) + 
  theme_bw() + 
  theme(strip.background = element_blank())
...
ggsave(
  "infection-modeling/graphics/tick-attachment-by-capture-timing.jpg",
  width = 7, height = 7*.33, dpi = 200
)
```

It also appears that individuals that are captured later in the night are less
likely to have larval ticks attached.

### tick density

```{r, fig.asp=0.8, fig.width=6}
event_tick_densities <- tick_densities |> 
  group_by(eventID, sexOrAge) |> 
  summarize(
    density = sum(density), sweeps = mean(sweeps), 
    count = sum(count), # density = count/sweeps
    .groups = "drop"
  ) # |> 
  # pivot_wider(names_from = sexOrAge, values_from = c(density, count))
...
full_data |> 
  ...
  left_join(
    event_tick_densities, 
    by = c("boutID" = "eventID"), relationship = "many-to-many"
  ) |> 
  filter(!is.na(sexOrAge), !sexOrAge %in% c("Adult", "NA")) |> 
  mutate(
    sexOrAge = factor(
      sexOrAge, levels = c("Male", "Female", "Nymph", "Larva")
    ),
    sexOrAge = forcats::fct_relabel(
      sexOrAge, function(lvl){gsub("(.*)", "\\1 density", lvl)}
    ),
    tickStage = factor(tickStage, levels = c("adult", "nymphal", "larval")),
    tickStage = forcats::fct_relabel(
      tickStage, function(lvl){
        gsub("(.*)", "P\\(\\1 attachment\\)", str_to_sentence(lvl))
      }
    )
  ) |> 
  ggplot(aes(x = density, y = attached)) +
  facet_rep_grid(tickStage ~ sexOrAge, scales = "free_x", switch = "both") + 
  geom_point(size = 1) +
  geom_smooth(
    formula = "y~x", method = "glm", method.args = list(family = "binomial")
  ) + 
  theme_bw() + 
  theme(
...
ggsave(
  "infection-modeling/graphics/tick-attachment-by-tick-density.jpg",
  width = 6, height = 6*0.8, dpi = 200
)
```

* The only apparent relationship between tick density and tick attachment, is
with larvae: larval tick density is strongly correlated with the probability
of a mouse having a tick attached when captured.
...
litter_loc_path = "infection-modeling/data/litter-trap-locations.rds"
if (!file.exists(litter_loc_path)) {
  litter_locations <- litter_prod$ltr_pertrap |> 
    ...
    geoNEON::getLocByName(token = NEON_TOKEN) |> 
...
ggsave(
  file.path(
    "infection-modeling/graphics/", 
    "siteMonthlySeedMass_2021-2024.jpg"
  ), 
...
ggsave(
  file.path(
    "infection-modeling/graphics/",
    "litterfall-mammal-trap-proximity.jpg"
  ),
...
full_data |> 
  ...
  labs(
    ...
    y = "Relative capture time (mean ± SE within site and year)"
  ) + 
...
ggsave(
  file.path(
    "infection-modeling/graphics/",
    "monthly-litter-captures_with-capture-time-windows.jpg"
  )
)
```
...
ggsave(
  file.path(
    "infection-modeling/graphics/", 
    "Capture-time-by-seedmass_within-year.jpg"
  ),
...
```{r}
# saveRDS(full_data, file = "infection-modeling/data/full-data.rds")
# TODO: update this... see file "00_big-table-prep.Rmd"
warning("UPDATE THIS FILE... RECONCILE WITH '00_big-table-prep.Rmd'")
```
